...
function makeCSVRow(j){
  return {
    ...
    "param_chaos":j._parsed_params?.chaos || "",
    "param_weird":j._parsed_params?.weird || "",
    "param_tile":j._parsed_params?.tile || false,
    "param_public":j._parsed_params?.public || false,
    "param_stealth":j._parsed_params?.stealth || false,
    "param_relax":j._parsed_params?.relax || false,
    "param_turbo":j._parsed_params?.turbo || false,
    "param_video":j._parsed_params?.video || false,
    "param_upanime":j._parsed_params?.upanime || false,
    "param_upbeta":j._parsed_params?.upbeta || false,
    "param_uplight":j._parsed_params?.uplight || false,
    "seedImageURL":j.event.seedImageURL || "",
    "width": j.event?.width || 0,
    "height": j.event?.height || 0,
  ...
  const settingsPanel = (() => {
    ...
    f.onsubmit = (ev) => {
      (async () => {
        ...
        if (imageFormat !== "csv") zipper.push({name: "metadata.jsonl", input: jsonlData.join("\n")+"\n" });
        if (imageFormat !== "jsonl") zipper.push({name: "metadata.csv", input: papa.unparse(csvData)});
